...
public class SimpleEventRaiserTest {
    ...
    public void testRaiseDummyEventWithSingleHandler() {
        ...
        when(contextMock.getBean(eq(handler1Name)))
            .thenReturn(handler1Mock);
        ...
        verify(contextMock, times(1)).getBean(eq(handler1Name));
        verify(handler1Mock, times(1)).handle(eq(event));
    }
    ...
    public void testRaiseDummyEventWithMultipleHandlers() {
        ...
        when(contextMock.getBean(eq(handler2Name)))
            .thenReturn(handler2Mock);
        ...
        verify(contextMock, times(1)).getBeanNamesForType(any(ResolvableType.class));
        verify(contextMock, times(1)).getBean(eq(handler1Name));
        verify(contextMock, times(1)).getBean(eq(handler2Name));
        verify(handler1Mock, times(1)).handle(eq(event));
        verify(handler2Mock, times(1)).handle(eq(event));
    }
